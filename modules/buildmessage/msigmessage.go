...
func (m *Msiger) NewMsigLockProposeMessage(baseParams BaseParams, msigAddress, startEpoch, unlockDuration, amount string, from string) (*types.Message, *multisig11.ProposeParams, error) {
	...
	params, actErr := actors.SerializeParams(&multisig11.LockBalanceParams{
...
func (m *Msiger) NewMsigThresholdProposeMessage(baseParams BaseParams, msigAddress, newThreshold string, from string) (*types.Message, *multisig11.ProposeParams, error) {
	...
	params, actErr := actors.SerializeParams(&multisig11.ChangeNumApprovalsThresholdParams{
		NewThreshold: newT,
	})
...
func serializeAddParams(new address.Address, inc bool) ([]byte, error) {
	enc, actErr := actors.SerializeParams(&multisig11.AddSignerParams{
		Signer:   new,
		Increase: inc,
	})
	if actErr != nil {
...
func serializeSwapParams(old address.Address, new address.Address) ([]byte, error) {
	enc, actErr := actors.SerializeParams(&multisig11.SwapSignerParams{
		From: old,
		To:   new,
	})
	if actErr != nil {
...
func serializeRemoveParams(rem address.Address, dec bool) ([]byte, error) {
	enc, actErr := actors.SerializeParams(&multisig11.RemoveSignerParams{
		Signer:   rem,
		Decrease: dec,
	})
	if actErr != nil {
...
func txnParams(id uint64, data *multisig11.ProposalHashData) (*multisig11.TxnIDParams, error) {
	params := multisig11.TxnIDParams{ID: multisig11.TxnID(id)}
	if data != nil {
